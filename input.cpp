        ...
        if (onechar) {
            ...
            if (hasDecimal && pos - start == 1) {
                throw std::runtime_error(
                    make_error_message(expression, start, "Invalid number format: lone decimal point"));
            }

            tokens.push_back({TokenType::Const, expression.substr(start, pos - start)});
        } else {
            throw std::runtime_error(
                make_error_message(expression, pos, "Invalid character encountered: " + std::string(1, c)));
        }
    }
...
std::unique_ptr<ASTNode> parse_tokens(std::vector<Token> tokens) {
    ...
    auto make_node = [](TokenType op, std::unique_ptr<ASTNode> right, std::unique_ptr<ASTNode> left) {
        ...
        switch (op) {
        case TokenType::Add: node->type = ASTNodeType::Add; break;
        case TokenType::Sub: node->type = ASTNodeType::Sub; break;
        case TokenType::Mul: node->type = ASTNodeType::Mul; break;
        case TokenType::Pow: node->type = ASTNodeType::Pow; break;
    ...
    for (size_t i = 0; i < tokens.size(); i++) {
        ...
        case TokenType::Const: {
            ...
            if (i + 1 < tokens.size() &&
                (tokens[i + 1].type == TokenType::Var || tokens[i + 1].type == TokenType::LPar ||
        ...
        case TokenType::LPar: operator_stack.push_back(token.type); break;
        case TokenType::RPar: {
