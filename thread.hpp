...
std::vector<RETVAL> parallel_map(const T& func, std::span<ARG> args) {
    ...
    std::function<RETVAL(ARG)> f = [func](ARG arg) -> RETVAL { return func(arg); };
    parallel_for(args.size(), [&](usize i) { result[i] = f(args[i]); });
    return result;
}

/// Apply the function `func` to each index from 0 to `n - 1` in parallel.
/// The results are collected into a vector.
template <typename RETVAL, typename T> std::vector<RETVAL> parallel_nat_map(usize n, const T& func) {
    std::vector<RETVAL> results;
    results.resize(n);
    std::function<RETVAL(usize)> f = [func](usize i) -> RETVAL { return func(i); };
    parallel_for(n, [&](usize i) { results[i] = f(i); });
    return results;
}